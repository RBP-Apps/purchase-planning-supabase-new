...
const VendorMasterSection = () => {
  ...
  const existingItemTypes = Array.from(new Set(records.map((r) => r.items_type).filter(Boolean)));
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-2.5 text-sm text-blue-600 break-all min-w-[300px]">{record.email}</td>
                    <td className="px-5 py-2.5 text-sm text-slate-500">{record.associated_modules}</td>
                    <td className="px-5 py-2.5 text-sm text-slate-500 break-words min-w-[250px]">{record.address}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <form id="vendor-form" onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      name="items_type" 
                      list="vendor-items-type-list"
                      value={formData.items_type} 
                      onChange={handleInputChange} 
                      className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none" 
                      placeholder="e.g. CABLE, PUMP, BOS" 
                    />
                    <datalist id="vendor-items-type-list">
                      {existingItemTypes.map((type, idx) => (
            ...
            <div className="p-6">
              <form id="item-form" onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <input 
                    name="item_type" 
                    list="item-master-type-list"
                    value={formData.item_type} 
                    onChange={handleInputChange} 
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none" 
                  />
                  <datalist id="item-master-type-list">
                    {Array.from(new Set(records.map((r) => r.item_type).filter(Boolean))).map((type, idx) => (
                      <option key={idx} value={type} />
                    ))}
                  </datalist>
                </div>
            ...
            <div className="p-6">
              <form id="term-form" onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <input 
                    name="item_type" 
                    list="terms-item-type-list"
                    value={formData.item_type} 
                    onChange={handleInputChange} 
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:bg-white focus:ring-1 focus:ring-blue-500 outline-none" 
                    required 
                    placeholder="e.g. STRUCTURE, CABLE" 
                  />
                  <datalist id="terms-item-type-list">
                    {Array.from(new Set(records.map((r) => r.item_type).filter(Boolean))).map((type, idx) => (
                      <option key={idx} value={type} />
                    ))}
                  </datalist>
